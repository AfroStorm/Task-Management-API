...
class NotifyDueDatesTest(TestCase):
    ...
    def setUp(self):
        ...
        self.task_group3.team_members.set([self.userprofile])
        ...
        # Task with an overdue date
        self.task_overdue = models.Task.objects.create(
            title='Task Overdue',
    ...
    def test_notify_due_dates_command(self):
        ...
        self.assertEqual(sent_email.from_email, 'admin@it-backends.com')
    ...
    def test_notify_overdue_dates_command(self):
        # Call the management command
        call_command('notify_overdue_dates')

        # Check if the email was sent to the correct recipient
        self.assertEqual(len(mail.outbox), 1)
        sent_email = mail.outbox[0]
        self.assertEqual(
            sent_email.subject, 'The date of the task is overdue!'
        )
        self.assertIn(str(self.task_overdue.due_date), sent_email.body)
        self.assertIn(self.task_overdue.title, sent_email.body)
        self.assertEqual(sent_email.from_email, 'admin@it-backends.com')
        self.assertEqual(sent_email.to, [self.userprofile.email])

        # Check if the email was not sent for the task not due soon
        self.assertNotIn(self.task_due_soon.title, sent_email.body)
